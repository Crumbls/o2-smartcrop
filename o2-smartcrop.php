<?php
/**
 * Plugin Name: O2Group SmartCrop
 * Plugin URI: https://o2group.com
 * Description: Based on the works of WP SmartCrop.
 * Version: 2.0.2
 * Author: O2Group
 * Author URI: https://o2group.com
 * License: GPLv2 or later
 * Text Domain: o2smartcrop
 **/

namespace O2Group\SmartCrop;

use O2Group\SmartCrop\Entity;
use O2Group\SmartCrop\Entity\Vector;
use O2Group\SmartCrop\Entity\Polygon;

defined('ABSPATH') || exit();


//use O2Group\Plugins\SmartCrop\Rectangle;
...
if (!class_exists('WP_Smart_Crop')) {
    class WP_Smart_Crop
    {
        ...
        function wp_smartcrop_select_focus_mode()
        {
            ...
            <label>
                ...
                <select name='wp-smartcrop-settings[focus-mode]'>
                    <option value="power-lines" <?php selected($focus_mode, 'power-lines'); ?>>Default (Power Lines)
                    </option>
                    <option value="relative-position" <?php selected($focus_mode, 'relative-position'); ?>>Relative
                        Position
                    </option>
                </select>
            </label>
        ...
        function wp_smartcrop_disable_thumbnail_generation()
        {
            ...
            ?>
            <input type='checkbox'
                   name='wp-smartcrop-settings[disable-thumbnails]' <?php checked($disable_thumbs, 1); ?> value='1'>
            ...
            if (substr($post->post_mime_type, 0, 5) == 'image') {
                ...
                <div class="wpsmartcrop_preview_wrap" style="max-width: <?php echo 3 * $width; ?>px;">
                    ...
                    <div class="wpsmartcrop_gnomon">
                        ...
                        <div class="wpsmartcrop_gnomon_c"
                             style="top:  <?php echo $focus['top']; ?>%; left: <?php echo $focus['left']; ?>%;"></div>
                    </div>
                </div>
                <?php
                ...
                ob_start();
                ?>
                <input type="checkbox" class="wpsmartcrop_enabled" id="wpsmartcrop_enabled"
                       name="attachments[<?php echo $post->ID; ?>][_wpsmartcrop_enabled]"
                ...
                <div class="wpsmartcrop_interface<?php echo $enabled_class; ?>">
                    ...
                    <input type="hidden" class="wpsmartcrop_image_focus_left"
                           name="attachments[<?php echo $post->ID; ?>][_wpsmartcrop_image_focus][left]"
                           value="<?php echo $focus['left']; ?>"/>
                    <input type="hidden" class="wpsmartcrop_image_focus_top"
                           name="attachments[<?php echo $post->ID; ?>][_wpsmartcrop_image_focus][top]"
                           value="<?php echo $focus['top']; ?>"/>
                                        ...
                                        <label>
                                            ...
                                            <span class="wpsmartcrop_percent_wrap">
												<input type="number" min="0" max="100" step="0.01"
                                                       class="wpsmartcrop_temp_focus_left" value=""/>
												<span>%</span>
											</span>
                                        </label>
                                        <label>
                                            ...
                                            <span class="wpsmartcrop_percent_wrap">
												<input type="number" min="0" max="100" step="0.01"
                                                       class="wpsmartcrop_temp_focus_top" value=""/>
												<span>%</span>
											</span>
                                        </label>
...
add_action('init', function () {
    ...
    function dumpR(Entity\Response $r)
    {
...
//pointInCircle
    ...
    // RETHINK TEH WHOLE THING.
    $a = new Entity\Rectangle(0, 0, $w, $h);

    $b = new Entity\Rectangle($w / 100 * $left - 140 / 2, $h / 100 * $top - 140 / 2, 140, 140);
    if (!$a->canFit($b)) {
        // Normally, you'd return here.
    } else {
        if (!$a->contains($b)) {
            // Does the width work?
            echo 'make an adjustment! ';
        }
        echo 'crop away! ';
    }
    echo PHP_EOL;
    ...
    $top = 26.13;
    $top = 1;
    $c = new Entity\Rectangle($w / 100 * $left - 140 / 2, $h / 100 * $top - 140 / 2, 140, 140);
    if (!$a->canFit($c)) {
        // Normally, you'd return here.
    } else {
        if (!$a->contains($c)) {
...
add_filter('wp_generate_attachment_metadata', function ($image_data, $id) {

    // On every photo, check to see what's what first.
    if (!$temp = get_post_meta($id, '_wpsmartcrop_image_focus', true)) {
        return $image_data;
    }
    // Invalid setting.
    if (!array_key_exists('left', $temp) || !array_key_exists('top', $temp)) {
        return $image_data;
    }
    // No need to edit. It's centered.
    if ($temp['left'] == 50 && $temp['top'] == 50) {
        return $image_data;
    }

    // Generate a box for our original image.
    $a = new Entity\Rectangle(0, 0, $image_data['width'], $image_data['height']);

    $left = $temp['left'];
    $top = $temp['top'];

    // How do we get our potential width and height? They go here.
//    if (sizeof($image_data)) =

    foreach ($image_data['sizes'] as $size => $sizeData) {
        $b = new Entity\Rectangle(
            $image_data['width'] / 100 * $left - $sizeData['width'] / 2,
            $image_data['height'] / 100 * $top - $sizeData['height'] / 2,
            $sizeData['width'],
            $sizeData['height']
        );
        if (!$a->canFit($b)) {
            // Thumbnail does not fit.  Bail.
            continue;
        }
        if (!$a->contains($b)) {
            // Adjust X.
        ...
        $upload_dir = wp_upload_dir();
        $location = $upload_dir['path'] . '/' . $image_data['sizes'][$size]['file'];

        // set our temp image file
        $locationTmp = $location.'.tmp';

        // get the attributes of the source image
        list($imageWidth, $imageHeight, $imageType, $imageAttr) = getimagesize($location);

        $staticWidth = $b->x2-$b->x;
        $staticHeight = $b->y2-$b->y;

        // Switch our handling depending on the image type.
        switch ($imageType) {
            //GIF
            case 1:
                ...
                imagegif($newimage, $locationTmp);

                // delete the uploaded image
                unlink($location);

                // rename the temporary brands image
                rename($locationTmp, $location);
            ...
            case 2:
                ...
                imagecopy($newimage, $src, $x_pos, $y_pos, 0, 0, $imageWidth, $imageHeight);
                imagejpeg($newimage, $locationTmp);
...
//        return $image_data;
    ...
    // If we are set to dead center, bail.

    // Determine if there is enough space based on our offsets to make this happen.
